...
- Airline (17 levels): unique 2-letter carrier code
...
### Data Preparation 
The non-response data has been named NA. In order to process this large data set more efficiently in RStudio during modeling, the author eliminated some missing data and randomly sampled 6000 observations from the sampled population of 618612 observations. In the following study, we will only use the 6000 newly sampled observations. Build a model using DayofWeek, Airline, AirTime to predict the delay probability of the flight. See the appendix or GitHub repository link for details on how the data set was cleaned and sampled. 
...
ggplot(plot2, aes(DAY_OF_MONTH, value)) +
  geom_line(aes(colour = status))+
  theme_minimal()+ggtitle("Delayed versus Cancelled Flights (Figure.2)")+xlab("day of month") + ylab("probability")+ theme(plot.title = element_text(size=8))
```

Figure.2: The delayed and canceled probabilities are combined in this figure. It is surprising to find that while passengers face the different probabilities of delays, the probabilities of canceled flights remain stables over the month. Even when the delayed number is quite high, there is only a slight change in canceled flights. The correlation coefficient is 0.234, which is smaller than the previous one (match with our finding). 


## Model
### Model Selection
A Bayesian logistic model is employed to address this study.Logistic regression is used to model the probability of a binary variable with only two potential outcomes labeled “0” and “1” while the predictor variables are continuous or categorical. The natural logarithm of an odd ratio, logit, is the central mathematical concept of the logistic regression. (Peng, Lee, & Ingersoll, 2002). There are three assumptions of the logistic regression model: First, binary regression requited the outcome variable to be binary; Second, the observation should be independent of each other. Third,there should be little or no multicollinearity among predictors; Fourth, independent variables have linear relationship with log odds; Fifth, large sample size is required to apply the logistic model. The logit can be stated as: 
...
- Convert odds to simply probability
$$P=\frac{e^{ \beta_0+\beta_1  X_{AirTime_ij} + \sum{\beta_i  X_{DayofWeek,i}} +\sum{\beta_j  X_{Airline,j}+\epsilon}}}{e^{1+\beta_0+\beta_1  X_{AirTime_ij} + \sum{\beta_i  X_{DayofWeek,i}} +\sum{\beta_j  X_{Airline,j}+\epsilon}}},  i=1,2...7, j=1,2,...17$$
...
ggplot(airline1, 
  ...
  theme_minimal()+ggtitle("Delayed Flights by Major Carriers (Figure.3)")+xlab("major carriers") + ylab("number")+ scale_fill_manual(values=wes_palette(n=4, name="FantasticFox1"))+ theme(plot.title = element_text(size=8))
```
...
ggplot(airline1,aes(x = DAY_OF_WEEK, fill = DELAY)) + 
  geom_bar(position = "stack")+
  theme_minimal()+ggtitle("FLight Delays of the Week (Figure.4)")+xlab("day of week") + ylab("number")+scale_fill_manual(values=wes_palette(n=4, name="Zissou1"))+ theme(plot.title = element_text(size=8))
```
...
library(kableExtra)
library(kableExtra)
yz_table <- data.frame("estimated coefficients" = c("-1.18","-0.43","-0.49","-0.51","-0.32","-0.45","-0.18","-0.28","0.12","0.54","-0.52","0.02","0.29","-0.00","-1.14","-0.21","0.00","0.07","-0.10","-0.13","0.17","0.21","-0.24","0.00"),"lower 95%CI"=c(" -1.52","-0.66","-0.73","-0.74","-0.55","-0.70","-0.38","-0.65","-0.33","0.12","-0.90","-0.57","-0.26","-0.61","-2.11","-0.65","-0.49","-0.40","-0.48","-0.53","-0.19","-0.26","-0.70","0.00"),"upper 95%CI"=c("-0.83","-0.21","-0.25","-0.28","-0.09","-0.21","0.03","0.08","0.56","0.95","-0.15","0.59","0.80","0.58","-0.30","0.24","0.48","0.52","0.27","0.27","0.51","0.68","0.21","0.00"))
...
table
```
Table.1: This table contains the regression results of the Bayesian logistic model. The respective coefficients are listed in the cells. Since P=1 if flight delays, the higher estimated coefficient means it is more likely to delay. Compared to Monday, all the coefficients are negative; thus, Monday has the highest probability to experience flight delays while Thursday has the lowest probability on average. 

As for the airlines, respective to Endeavor Air (9E),  Alaska Airlines, JetBlue Airways, Atlantic Southeast Airlines, Frontier Airlines, Southwest Airlines, and Mesa Airlines have worse on-time performance. Since JetBlue Airways’s coefficient is the largest (0.54), the flights scheduled by JetBlue is the most likely to delay. On the other hand, Hawaiian Airlines (-1.14) is predicted to have the highest on-time rate compared to other major carriers. Also, as discussed above, Southwest Airlines has the highest number of total delayed flights but according to our results, Southwest Airlines does not have a higher probability to delay. The total number of delay is mainly contributed by the number of scheduled flights. Therefore, it may be one of the reasons that most domestic passengers in the United States choosing Southwest Airlines. 

Longer flights come with more retributions and regulations that airlines need to deal with. Also, since their routes involved multiple airspaces and airlines need to corporate with more conditions (such as the changing weather along the way). However, it is interesting to find that the airline does not show any significant impact on flight delays. To minimize the potential cost, the best case is purchasing a flight that flys on Thursday carried by Hawaiian Airlines. Try to avoid the worst scenario, booking a flight that is scheduled on Monday carried by JetBlue Airways. 

```{r,echo=FALSE,include=FALSE,message=FALSE,warning=FALSE}
stanplot(model,type = "areas",prob = 0.95)
```
...
## Discussion
### Model Summary 
Most of the model assumptions is satisfied. From the model results, we find that flight delay probabilities vary across the day of the week and different airline carriers, but it is surprising to find that flight time has no significant contribution to determine flight delays. Since CIs for most of the variables in the population level do not contain zero (means p-value <0.05), there is strong statistical evidence to illustrate that flight delays are highly associated with day of the week. As the estimated coefficients shown in the table, we can compute the odd ratio for each category; thus, we can access the delay probabilities of all flights in general. To be more precise, Monday has the highest probability (P=0.235) to encounter flight delays while Thursday is the lowest (p=0.149). Also, choosing airlines is essential to flight delays. In our results, purchasing a flight ticket of JetBlue Airways (B6) is the most likely to delay while Delta Airlines has the best on-time performance on average.

## Conclusion 
This paper presents a single-level analysis for the on-time performance of commercial flights in the United States. Our results indicate that the on-time departure probability depends significantly on the day of the week and airline carriers. According to the estimation, Monday has the highest probability to delay at the population-level for all airlines. One hypothesis is that airlines and airports are experiencing the highest volume of flights among seven days of the week, making it hard to dispatch aircrafts. For passengers, it can be a little more strategic on buying flight tickets. To minimize your chances of delays, book your flights on Wednesday, Thursday,and Saturday or choose the carriers such as Delta Airlines, Hawaiian Airlines, and Alaska Airlines if possible. If you cannot avoid traveling on Monday, purchasing travel insurance is worth considering. 
...
library(brms)
set.seed(2020)
model <- brm(DEP_DEL15 ~ DAY_OF_WEEK +OP_UNIQUE_CARRIER+AIR_TIME,data=dff,
...
# make a table for the output
library(kableExtra)
yz_table <- data.frame("estimated coefficients" =c("-1.18","-0.43","-0.49","-0.51",
               "-0.32","-0.45","-0.18","-0.28","0.12","0.54","-0.52",
               "0.02","0.29","-0.00","-1.14","-0.21","0.00","0.07",
               "-0.10","-0.13","0.17","0.21","-0.24","0.00"),
  "lower 95%CI"=c(" -1.52","-0.66","-0.73","-0.74","-0.55","-0.70","-0.38",
                  "-0.65","-0.33","0.12","-0.90","-0.57","-0.26","-0.61",
                  "-2.11","-0.65","-0.49","-0.40","-0.48",
                  "-0.53", "-0.19","-0.26","-0.70","0.00"),
  "upper 95%CI"=c("-0.83","-0.21","-0.25","-0.28","-0.09","-0.21","0.03",
                  "0.08","0.56", "0.95","-0.15","0.59","0.80","0.58","-0.30",
                  "0.24","0.48","0.52","0.27","0.27","0.51","0.68","0.21","0.00"))

row.names(yz_table) <- c("Intercept","Tuesday","Wednesday","Thursday","Friday","Saturday",
                         "Sunday","American Airlines (AA)","Alaska Airlines (AS)",
...
table <- yz_table %>%
  kbl(caption = "Population-Level Effects (Table.1)") %>%
  kable_styling(bootstrap_options = "striped", position = "center",full_width = F) %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```

```{r,echo=TRUE,message=FALSE,warning=FALSE,fig.height=3,fig.width=8}
